namespace Compiler.Expressions.MathOperations
{
    ...
    {
        public MultiplyOperation(DataType type, MathExpression lhs, MathExpression rhs) : base(Op.Multiply, type, lhs, rhs)
        {
        ...
        { 
            ...
            {
                result = null;
                return $"can't multiply types {lhs.Type} and {rhs.Type}";
            }
            ...
            if (lhs.Type == DataType.UInt || rhs.Type == DataType.UInt)
            {
                ...
                result = new MultiplyOperation(DataType.UInt, lhs, rhs);
                return null;
            }
            else
            {
                result = new MultiplyOperation(DataType.UByte, lhs, rhs);
                return null;
            }
        }
        ...
        private MathCalculation CompileIntInt(CodeWriter writer, Definitions definitions)
        {
                ...
                right.StorageType == MathCalculation.Type.DataValue)
            {
                ...
                if (right.StorageType == MathCalculation.Type.Imediate)
                {
                    ...
                    writer.WriteLine("mul bx");
                }
                else
                    writer.WriteLine($"mul {right.Value}");
            }
            else if (right.StorageType == MathCalculation.Type.Register)
            {
                if (!leftPushed)
                {
                    if (left.Value == "ax")
                        ...
                        writer.WriteLine($"mul {left.Value}");
                    else
                    {
                        writer.WriteLine($"mov ax, {left.Value}");
                        writer.WriteLine($"mul {right.Value}");
